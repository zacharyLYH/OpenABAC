...
import AttachPageSearch, { AttachPageSearchProps } from "@/components/attach-page-components/search";
...
const AttachEntity = () => {
    ...
    const removeRight = {
        ...
        selected: leftSelected,
        setSelected: setLeftSelected,
        mountIndicator: left.length > 0 && right.length > 0 && rightSelected.length > 0,
        placeholder: `Select ${left}...`,
        maxAllowSelect: 2000
    }
    return (
        <section>
            ...
            {addOrRemove && left && right &&
                <div className="flex flex-row w-full">
                    <div className="flex items-center justify-center w-full border rounded-xl mx-3">
                        {addOrRemove === 'Add' ? attachPageSearchGenerator(addLeft) : attachPageSearchGenerator(removeLeft)}
                    </div>
                    <div className="flex items-center justify-center w-full border rounded-xl mx-3">
                        {left.length > 0 && right.length > 0 ? (
                            (addOrRemove === 'Add' && leftSelected.length > 0) ?
                                attachPageSearchGenerator(addRight) :
                                (addOrRemove === 'Remove' && rightSelected.length > 0) ?
                                    attachPageSearchGenerator(removeRight) :
                                    <p>👈 Select {addOrRemove === 'Add' ? left : right} to {addOrRemove === "Add" ? "add to" : "remove"} {addOrRemove === 'Add' ? right : left + " from"}</p>
                        ) : (
            ...
            <div className={cn("flex justify-center items-center my-4", !(addOrRemove && left && right && leftSelected.length > 0 && rightSelected.length > 0) && 'cursor-not-allowed')}>
                <Button
