...
import { ABACRequestResponse, Action, Context, Query, User } from '@/abac/interface';
...
export async function authorizeActionNameGivenApplicationUserId(
   ...
   for(const ctx of contextAssociated){
    ...
    if(ctx.operator === "BETWEEN"){
        ...
        if (!(timeValue1 instanceof Date && !isNaN(timeValue1.getTime()) && 
            timeValue2 instanceof Date && !isNaN(timeValue2.getTime()) && 
            currentTime > timeValue1 && currentTime < timeValue2)
        ) {
            return {
                authorized: false,
    ...
    if([">", "<", ">=", "<=", "==", "!="].includes(ctx.operator)){
        const valueToCompare = ctx.timeValue1 ? ctx.timeValue1 : ctx.textValue
        if(!valueToCompare){
            return {
                authorized: false,
